...
- Upload large files
- Upload files and entire folders directly or via drag-and-drop
- WebDAV endpoint (supports scoped access tokens)
- Create folders
- Search files
- Image / video / PDF thumbnails
- Preview images, videos, Word, Excel, and PowerPoint files
...
To require a Cloudflare Turnstile challenge before password login, create a
Turnstile widget and set both `FLAREDRIVE_TURNSTILE_SITE_KEY` and
`FLAREDRIVE_TURNSTILE_SECRET_KEY`. The login API verifies the Turnstile token
before decrypting and checking the password. WebDAV clients continue to use
Basic Auth or scoped access tokens under `/webdav`.
...
1. Fork this project and connect your fork with Cloudflare Pages
   ...
   - (Optional) Set `FLAREDRIVE_AUTH_MODE` to `password`, set
     `FLAREDRIVE_LOGIN_ACCOUNT` and `FLAREDRIVE_LOGIN_PRIVATE_KEY`, optionally
     set `FLAREDRIVE_TURNSTILE_SITE_KEY` and
     `FLAREDRIVE_TURNSTILE_SECRET_KEY`, then bind a D1 database as `AUTH_DB`
