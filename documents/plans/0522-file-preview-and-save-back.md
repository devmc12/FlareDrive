...
- Support direct ViewerJS image viewing, text/Markdown save-back, ZIP listing, native audio/video preview, and higher-fidelity local Office preview.
...
- Run `npm run build`.
- Confirm build output keeps common preview dependencies in `preview-vendors`, keeps Office preview dependencies in separate lazy Word/spreadsheet chunks, and does not create extra manual React/MUI chunks.
- Confirm expected asset groups are the app entry, `preview-vendors`, lazy `preview-pdf-vendors`, lazy `preview-word-vendors`, lazy `preview-spreadsheet-vendors`, `pdf.worker`, and the small Rollup/Rolldown runtime.
...
- Browser-check `http://127.0.0.1:3601`:
  ...
  - ZIP, Univer spreadsheet preview, and docx-preview rendering plus fallback behavior match the size limits
...
- Office preview dependencies remain lazy because Univer and docx-preview are much heavier than the rest of the preview stack.
